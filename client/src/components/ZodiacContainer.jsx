import React from "react";
// import {useSelector} from "react-redux"

import ZodiacCardsList from "./ZodiacCardsList";
// import lang from "../utils/langConstants";

const ZodiacContainer = () => {

  // const LangKey = useSelector(store => store.configApp.lang)

  return (
    <div className="flex-col px-4 lg:px-14 flex justify-center lg:my-10 items-start relative w-12/12 opacity-90">
      <div className="lg:h-[50vh] px-4 lg:px-10 flex flex-col justify-center  items-center 2xl:my-4  lg:my-14 pt-6 lg:pt-24 py-4 rounded-2xl w-full">
        <div className="flex justify-center items-center flex-col">
          {/* <span className="lg:text-3xl 2xl:text-6xl text-2xl text-center lg:font-semibold  font-[600] text-purple-300 pb-1.5 lg:pb-3 tracking-wider opacity-90 lg:tracking-normal">{lang[LangKey].horoTitle}</span> */}
          <span className="lg:text-3xl 2xl:text-6xl text-2xl text-center lg:font-semibold  font-[600] text-purple-300 pb-1.5 lg:pb-3 tracking-wider opacity-90 lg:tracking-normal">Daily Horoscope</span>
          <img alt="line"
            className="lg:w-72 2xl:w-80 w-60"
            src="data:img/png;base64,"
          ></img>
        </div>
        <div className="w-full flex lg:justify-center items-center pt-4 lg:text-center text-left lg:pt-8 ">
            {/* <span className="text-purple-200 lg:w-10/12 opacity-90 font-normal 2xl:text-3xl   lg:text-lg tracking-wider text-[15px] lg:tracking-wide">{lang[LangKey].horoDesc}</span> */}
            <span className="text-purple-200 lg:w-10/12 opacity-90 font-normal 2xl:text-3xl   lg:text-lg tracking-wider text-[15px] lg:tracking-wide">Dive into the mystical world of astrology with our comprehensive horoscope section. Discover the secrets written in the stars and unlock insights into your personality, relationships, and future. Whether you're a seasoned astrology enthusiast or just curious about what the cosmos has in store for you, our horoscope section is your guide to understanding the celestial forces shaping your life.</span>
        </div>
      </div>
      <ZodiacCardsList />
    </div>
  );
};

export default ZodiacContainer;
